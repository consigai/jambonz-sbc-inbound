...
module.exports = (srf, logger) => {
  ...
  const wasOriginatedFromCarrier = async(req) => {
    ...
    /**
     * SBC_ACCOUNT_REALM: normalize the Request-URI host for account/carrier lookup.
     *
     * When DNS aliases or load balancers sit in front of the SBC, the Request-URI
     * host may not match the account's configured SIP realm (e.g., a customer-facing
     * hostname like sip.customer.example.com instead of sip.cluster.example.com).
     * Normalize to the configured realm so the account and carrier lookups succeed.
     */
    if (process.env.SBC_ACCOUNT_REALM && uri.host !== process.env.SBC_ACCOUNT_REALM) {
      logger.info({original_realm: uri.host, normalized_realm: process.env.SBC_ACCOUNT_REALM},
        'normalizing Request-URI realm to SBC_ACCOUNT_REALM');
      uri.host = process.env.SBC_ACCOUNT_REALM;
    }
